...
namespace Gaia
{
    ...
    class Gaia2TopPanel
    {
        ...
        {
            double range = GaiaTerrainLoaderManager.Instance.GetLoadingRange();
            Vector3Double currentPos = GaiaTerrainLoaderManager.Instance.GetOrigin();

            bool currentGUIState = GUI.enabled;

            float sizeX = GaiaSettings.m_gaiaPanelSizeType == HeightMaskType.Relative ? Camera.current.pixelRect.size.x * GaiaSettings.m_gaiaPanelSize.x / 100f : GaiaSettings.m_gaiaPanelSize.x;
            float sizeY = GaiaSettings.m_gaiaPanelSizeType == HeightMaskType.Relative ? Camera.current.pixelRect.size.y * GaiaSettings.m_gaiaPanelSize.y / 100f : GaiaSettings.m_gaiaPanelSize.y;
            float x = GaiaSettings.m_gaiaPanelPositionType == HeightMaskType.Relative ? (Camera.current.pixelRect.size.x * GaiaSettings.m_gaiaPanelPosition.x / 100f) - sizeX / 2f : GaiaSettings.m_gaiaPanelPosition.x;
            float y = GaiaSettings.m_gaiaPanelPositionType == HeightMaskType.Relative ? (Camera.current.pixelRect.size.y * GaiaSettings.m_gaiaPanelPosition.y / 100f) : GaiaSettings.m_gaiaPanelPosition.y;
                ...
                EditorGUILayout.BeginVertical(GUIStylePanel, GUILayout.MaxWidth(sizeX));
                {
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        if (GaiaTerrainLoaderManager.Instance.TerrainSceneStorage.m_hasWorldMap)
                        {
                            ...
                            {
                                if (WorldMapTerrain.isActiveAndEnabled)
                                {
                                    if (GUILayout.Button("Switch to Terrain", GUILayout.Height(15), GUILayout.Width(150)))
                                    {
                                ...
                                {
                                    if (GUILayout.Button("Switch to World Map", GUILayout.Height(15), GUILayout.Width(150)))
                                    {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    if (range != GaiaTerrainLoaderManager.Instance.GetLoadingRange())
                    {
                        GaiaTerrainLoaderManager.Instance.SetLoadingRange(range);
                    }
                    if (currentPos != GaiaTerrainLoaderManager.Instance.GetOrigin())
                    {
                        GaiaTerrainLoaderManager.Instance.SetOrigin(currentPos);
                    }
                }
            }
        }
