...
namespace Gaia
{
    ...
    public class SpawnerEditor : PWEditor, IPWEditor
    {
        ...
        private bool m_createBaseTerrainUnfolded;
...
#region SPAWNER MASK LIST MANAGEMENT
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m_spawnRulesHeader == null || m_singleSpawnRuleHeader == null || m_spawnRulesHeader.normal.background==null || m_singleSpawnRuleHeader.normal.background == null)
            {
                ...
                if (EditorGUIUtility.isProSkin)
                {
                    m_spawnRulesHeader.normal.background = GaiaUtils.GetBGTexture(GaiaUtils.GetColorFromHTML("2d2d2dff"), new List<Texture2D>()); 
                    m_singleSpawnRuleHeader.normal.background = GaiaUtils.GetBGTexture(GaiaUtils.GetColorFromHTML("2d2d4cff"), m_tempTextureList);
                }
                // or Unity Personal
                else
                {
                    m_spawnRulesHeader.normal.background = GaiaUtils.GetBGTexture(GaiaUtils.GetColorFromHTML("a2a2a2ff"), new List<Texture2D>());
            ...
            if (m_spawner.m_settings.m_isWorldmapSpawner)
            {
                m_createBaseTerrainUnfolded = m_editorUtils.Panel("CreateBaseTerrain", DrawCreateBaseTerrain, !m_spawner.m_baseTerrainStamped);
                DrawSpawnRulesPanel("SpawnStamps", DrawStampSpawnRules, m_spawner.m_baseTerrainStamped);
                DrawSpawnRulesPanel("DefineWorldBiomeMasks", DrawBiomeMaskSpawnRules, m_spawner.m_baseTerrainStamped);
                m_editorUtils.Panel("ExportTerrain", DrawExportTerrain);
        ...
        private void DrawCreateBaseTerrain(bool showHelp)
        {
        ...
        private void DrawExportTerrain(bool showHelp)
        {
            ...
            if (m_editorUtils.ButtonAutoIndent("ExportTerrainButtonExport"))
            {
                try
                {
                    ...
                    //Force a terrain loading update
                    GaiaTerrainLoaderManager.Instance.SwitchToLocalMap();
                    GaiaTerrainLoaderManager.Instance.SetLoadingRange(GaiaTerrainLoaderManager.Instance.GetLoadingRange());
        ...
        private bool DrawSpawnRulesPanel(string nameKey, Action<bool> contentMethod, bool unfolded = true)
        {
            ...
            switch (contentMethod.Method.Name)
            {
                case "DrawBiomeMaskSpawnRules":
                    unfolded = m_spawner.m_settings.m_spawnerRules.Where(x=>x.m_resourceType == SpawnerResourceType.WorldBiomeMask).Count() > 0 ? m_spawner.m_biomeMaskPanelUnfolded : true;
                    break;
        ...
        private void DrawBiomeMaskSpawnRules(bool helpEnabled)
        {
            ...
            GUILayout.Space(10);
            EditorGUILayout.BeginHorizontal();
            if (m_editorUtils.Button("ButtonCreateBiomes"))
            {
                BoundsDouble bounds = new BoundsDouble();
                TerrainHelper.GetTerrainBounds(ref bounds);
                ...
                for (int i = 0; i < allBiomeMaskRules.Length; i++)
                {
                    SpawnRule sr = allBiomeMaskRules[i];
                    if (sr.m_isActive)
                    {
                        BiomePreset biomePreset = m_spawner.m_settings.m_resources.m_worldBiomeMaskPrototypes[sr.m_resourceIdx].m_biomePreset;
                        ...
                        if (biomePreset != null)
                        {
                            ...
                            biomeController.m_settings.m_imageMasks = new ImageMask[1] { new ImageMask { m_operation = ImageMaskOperation.WorldBiomeMask, m_selectedWorldBiomeMaskGUID = sr.GUID } };
                        }
                    }
                }
            }
            //if (m_editorUtils.Button("ButtonSpawnBiomes"))
            //{
            //    SpawnRule[] allBiomeMaskRules = m_spawner.m_settings.m_spawnerRules.Where(x => x.m_resourceType == SpawnerResourceType.WorldBiomeMask).ToArray();
